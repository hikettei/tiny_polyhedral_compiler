...
def create_conv_schedule(N, K_out, H_out, W_out, Cin, KH, KW):
    ...
    # Writes Out (Accumulation)
    writes = I.UnionMap(
        f"{{ S_conv[n, k, h, w, c, kh, kw] -> Out[n, k, h, w] : "
        f"0<=n<{N} and 0<=k<{K_out} and 0<=h<{H_out} and 0<=w<{W_out} }}"
    )
    # Reads Out (Accumulation)
    reads_acc = I.UnionMap(
        f"{{ S_conv[n, k, h, w, c, kh, kw] -> Out[n, k, h, w] : "
        f"0<=n<{N} and 0<=k<{K_out} and 0<=h<{H_out} and 0<=w<{W_out} }}"
    )
    # Reads In/Weight omitted for simplicity (focused on Conv->Pool dependency)
...
def main():
    print("=== Conv2D + Pool2D Fusion V2 (PolyhedralSchedule API) ===\n")
    ...
    with I.context():
        ...
        # Transformations
        # We need to operate on psched.get_root()
